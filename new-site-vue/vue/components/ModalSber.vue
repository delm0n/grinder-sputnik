<template>
  <div id="modal-sber" class="modal-sber modal" style="display: none">
    <button class="modal-sber__close close">Закрыть</button>
    <div class="modal-sber__container">
      ...
      <div class="modal-sber__column">
        ...
        >
          ...
          <form-helicon :main="true" :dops="false" :credit="true" />
          <div class="modal-sber__price" v-for="item in getMain" :key="item.id">
            <div v-for="(subItem, index) in credit" :key="index">
              <p v-if="item.selected && subItem.selected">
                {{ Math.round(item.price / subItem.id).toLocaleString() }}
                РУБ/МЕС <span>+&nbsp;22.9%&nbsp;годовых</span>
              </p>
            </div>
          </div>
...
export default defineComponent({
    ...
    setup() {
        ...
        const changeCredit = (id) => store.commit('changeCredit', id)
        const getMain = computed(() => store.getters.getMain)
        const credit = computed(() => store.state.credit)
        return {
            changeMainProduct, changeCredit, getMain, credit
        };
    },
    methods: {
